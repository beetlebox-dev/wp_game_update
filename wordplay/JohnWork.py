...
from game import game, get_game_graph, prune_game_data
...
for _ in range(10):
    ...
    game_tree_result = get_game_graph(wordnet_data, i_synsets_by_depth, i_start_synset_num, hp)
    j_game_tree = game_tree_result[0]
    j_dead_ends = game_tree_result[1]

    pruned_game_data = prune_game_data(j_game_tree, i_synsets_by_depth, j_dead_ends, i_start_synset_num)
    i_game_tree = pruned_game_data[0]
    i_synsets_by_depth = pruned_game_data[1]  # todo changing to new index
    i_start_synset_num = pruned_game_data[2]  # todo changing to new index
    i_dead_ends = pruned_game_data[3]
    i_new_index_by_wordnet_index = pruned_game_data[4]
