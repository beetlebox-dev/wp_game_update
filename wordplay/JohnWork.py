...
for _ in range(10):
    ...
    i_lateral_connections = this_game_data[1]
    i_non_lateral_connections = this_game_data[2]
    i_start_synset_num = this_game_data[3]
    ...
    decoy_index /= len(game_tree)  # todo divide index by number of synsets in tree.

    ratio = round(one_decoys / len(game_tree), 2)
...
def print_synset_info_wn(synset_num, prefix=''):
    # i.e. 2 | throw, chuck | To toss
...
while True:
    ...
    for pointer in wordnet_data[wn_current_synset_num][4]:
        ...
        print_synset_info_wn(pointer_synset_id, pointer_prefix)
        pointer_num += 1
