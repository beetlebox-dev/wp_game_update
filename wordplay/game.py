...
def get_tree_for_game(wordnet_data, start_synset_id, depth):
...
#         return connection_after_prune
    ...
    # start_pointers_list = []
    # # lower_case_words = [word.lower().split('(')[0] for word in wordnet_data[start_synset_id][3]]
    # # word_index = lower_case_words.index(start_word)
    # child_pointer = ('__start', start_synset_id, 0, 0)
    # start_pointers_list.append(child_pointer)
    # start_pointers_list = [('__start', start_synset_id, 0, 0)]
    ...
    for _ in range(depth):  # Each loop is one layer in a breadth-first search.
        ...
        for parent_group in path_memory[0][-1]:  # Getting pointer groupings from previous generation.
            for parent_pointer in parent_group:  # Find neighbor_synsets of synsets_currently_visiting.
                ...
                for child_pointer in wordnet_data[parent_synset_id][5]:
...
# def web_app_inquiry(wordnet_data, wordnet_index, start_word, start_synset):
#
...
#             return result
#
#         tree_result = get_tree_for_game(
#             wordnet_data, formatted_start_word, start_synsets)
#
#     else:
#         tree_result = get_tree_for_game(
#             wordnet_data, formatted_start_word, [int(start_synset)])
#
...
def game(wordnet_data):
    ...
    tree = get_tree_for_game(wordnet_data, rand_synset_id, 3)
    print(tree)
    return tree
