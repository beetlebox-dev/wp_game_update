...
# todo split get_game_data into get_synsets_by_depth and get_start_synset.


def get_game_data(wordnet_data, target_synset_id, analysis_depth, start_depth):

    # Generate synsets_by_depth.
    ...
    # Select best_start_synset_id as the synset at start_depth with the most pointers.
    ...
    for _ in range(10):
        try:
            ...
            # todo debugging
            exception_str = f'Error choosing random synset from synsets_by_depth[start_depth].\n' \
                            f'synsets_by_depth: {synsets_by_depth}\nstart_depth: {start_depth}\n{e}'
            raise Exception(exception_str)
        pointer_count = len(wordnet_data[rand_start_synset][4])
        if pointer_count > highest_pointer_count:
            best_start_synset_id = rand_start_synset
            highest_pointer_count = pointer_count

    return synsets_by_depth, best_start_synset_id
